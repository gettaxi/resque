$(function() {
  ...
  ///////////////// Latency Support /////////////////////
  $.fn.latency = function() {
    return parseFloat(this.attr('data-latency'))
  }
 
  var render_latency_arrows = function(node) {
    node.find('.latency').each(function() {
      previous_latency = $('.latency#' + $(this).attr('id')).latency();
      current_latency = $(this).latency();
      var arrow_html;
      if ((previous_latency === null) ||
          ((current_latency < 1) && (previous_latency < 1)) ||
          (previous_latency == current_latency)) {
        arrow_html = '<span class="arrow">&nbsp;</span>'; 
      } else {
        arrow_direction = (previous_latency < current_latency) ? 'up' : 'down';
        arrow_html = '<span class="arrow ' + arrow_direction + '">' + 
                      ((arrow_direction == 'up') ? '&#10548;' /*up*/ : '&#10549;' /*down*/) + 
                      '</span>';
      }
      $(this).html($(this).html() + arrow_html);
    });    
  }
  ///////////////////////////////////////////////////////
